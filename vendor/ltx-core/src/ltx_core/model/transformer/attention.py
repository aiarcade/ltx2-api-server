...
class PytorchAttention(AttentionCallable):
    def __call__(
        ...
        # On sm75 (T4), mem_efficient_sdp only works with float16, not bfloat16.
        # Cast q/k/v to float16 for the SDPA call so mem_efficient_sdp is selected
        # instead of the O(N²) math path that OOMs at 1024×576.  The output is
